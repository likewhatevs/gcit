...
// `gcit::log::init`. The binary's `init_log` closure calls
// `log::init(filter, foreground)` for every subcommand; an invalid
// filter string is mapped to EX_CONFIG=78. Pinning these paths
// covers the EnvFilter::try_new error arm in log::init without
// needing to hold the global tracing subscriber state across
...
/// EX_CONFIG per cli::exit::CONFIG. Kept as a literal `i32` so
...
/// on anything else. The filter is consumed before config-load (per
/// the init_log invocation in bin/gcit.rs), so a filter rejection
...
fn invalid_log_filter_leading_equals_exits_config_78() {
    // log::init calls EnvFilter::try_new(raw) and maps any error to
    // InvalidInput. The binary's init_log closure prints "gcit: log
    // init failed: ..." and returns ExitCode::from(CONFIG). Per
    // tracing-subscriber's directive grammar, the first char must be
    // `[`, `-`, `:`, `_`, or alphanumeric — `=` is NOT in that set,
    // so a directive starting with `=` fails parse with ParseError.
    // Pin that the resulting exit code is 78 with the canonical
    // operator-facing "log init failed" lead.
