...
// swap on whether any flow has a `local_mail` destination. All other
...
// hardening in either branch. ReadWritePaths=/var/mail is emitted
// unconditionally — the line is harmless when no local_mail is in
// use and required when it is.
//
...
#[test]
fn unit_emits_read_write_paths_var_mail_unconditionally() {
    // The ReadWritePaths=/var/mail directive is emitted in both
    // variants. When local_mail is absent it is harmless (the
    // daemon never writes to /var/mail under DynamicUser); when
    // local_mail is present it is required (without it the unit's
    // ProtectSystem=strict + sandboxing would block writes to
    // /var/mail).
    //
    // Mutation target: implementer makes the line conditional on
    // local_mail; works for the with-local_mail case but operators
    // who flip a discord-only config to local_mail later get a
    // surprise sandbox failure on the next start.
    ...
    assert!(
        ...
        "no-local-mail unit must include ReadWritePaths=/var/mail; unit:\n{unit_no}",
    );
    assert!(
